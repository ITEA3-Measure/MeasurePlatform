...
public interface ProjectRepository extends JpaRepository<Project,Long> {
    ...
    @Query(value = "SELECT id, login, first_name, last_name, email FROM jhi_user INNER JOIN user_invited_project ON jhi_user.id = user_invited_project.user_id WHERE user_invited_project.project_id = ?1 UNION SELECT id, login, first_name, last_name, email FROM jhi_user INNER JOIN user_managed_project ON jhi_user.id = user_managed_project.user_id WHERE user_managed_project.project_id = ?1", nativeQuery = true)
    List<Object[]> findAllUsersByProjectId(@Param("projectId") Long projectId);

(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('measurePlatformApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-measurePlatformApp-error');
                var entityKey = httpResponse.headers('X-measurePlatformApp-params');
                if (errorHeader) {
